...
public:
    ...
    template<typename T>
    /// Возвращает указатель на компонент данного типа. Если такового нет
    ...
    template<typename T>
    /// Добавляет в объект компонент данного типа и возвращает указатель на него.
    ...
    /// Возвращает указатель на дочерний объект с данным именем. Если
    /// такового не существует, возвращает `nullptr`.
    ///
    /// - Parameter name: Имя искомого объекта
    ///
    ...
    /// Возвращает имя данного объекта.
    std::string get_name();
    
    /// Возвращает уникальный номер данного объекта.
    IntField get_id();
    
    /// Возвращает указатель на родительский объект данного объекта. Если
    /// данный объект корневой, то будет возвращён `nullptr`.
    std::shared_ptr<Object> get_parent();
    
    /// Возвращает упакованные данные объекта и его компонентов.
    PackedData pack();
    
    /// Возвращает изменения объекта и компонентов с прошлового вызова данного метода.
